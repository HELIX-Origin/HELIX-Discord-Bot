...
export const bakaCommandDef: ApplicationCommand = {
  name: 'baka',
  description: 'Call someone a baka with a GIF',
  options: [
    {
      name: 'user',
      description: 'The user to call baka',
...
export async function handleBakaCommand(
  ...
  const gifUrl = await client.getCategoryGif('baka');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${targetId}> B-Baka!` : '*B-Baka!*';
