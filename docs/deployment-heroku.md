...
Deploy HELIX to Heroku using the native Node.js buildpack (`Procfile` + `app.json`) on a single Eco Dyno with zero required paid add-ons.
...
- **Stack**: Native Node.js (`heroku/nodejs` buildpack via `Procfile`)
- **Dyno Tier**: Eco Dyno (`formation: web: eco`)
...
flowchart TD
    ...
    Inputs --> Build["3. Heroku Builds using heroku/nodejs Buildpack"]
    Build --> Launch["4. Boots Bot Gateway Client & Dashboard Server via Procfile"]
...
| `NODE_ENV` | Runtime environment | Set to `production` | Pre-configured in `app.json` |
