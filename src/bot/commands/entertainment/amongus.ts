...
export const amongusCommandDef: ApplicationCommand = {
  name: 'amongus',
  description: 'Get an Among Us GIF',
  options: [],
};

export async function handleAmongUsCommand(
  ...
  const gifUrl = await client.getCategoryGif('amongus');
  if (!gifUrl) {
...
export const amongusCommand: BotCommand = {
  def: amongusCommandDef,
  category: 'entertainment',
  isEnabled: (deps) => Boolean(deps.config.features.gifsEnabled && deps.config.klipyApiKey),
  execute: handleAmongUsCommand,
};
