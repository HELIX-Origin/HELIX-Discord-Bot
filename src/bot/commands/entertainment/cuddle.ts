...
export const cuddleCommandDef: ApplicationCommand = {
  name: 'cuddle',
  description: 'Cuddle someone with a GIF',
  options: [
    {
      name: 'user',
...
export async function handlecuddleCommand(
  ...
  const gifUrl = await client.getCategoryGif('cuddle');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${actor?.id ?? 'someone'}> cuddled <@${targetId}>!` : '*cuddled*';
...
export const cuddleCommand: BotCommand = {
  def: cuddleCommandDef,
  category: 'entertainment',
  isEnabled: (deps) => Boolean(deps.config.features.gifsEnabled && deps.config.klipyApiKey),
  execute: handlecuddleCommand,
};
