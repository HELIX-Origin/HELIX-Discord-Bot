...
Deploy HELIX Discord Bot to [Koyeb](https://www.koyeb.com/) on the **100% Free Nano Instance tier** using the native Dockerfile and declarative configuration (`koyeb.yaml`).
...
```mermaid
flowchart TD
    Button["1. Click Deploy to Koyeb Button"] --> Config["2. Koyeb Pre-Fills Dockerfile & Free Instance"]
    Config --> Inputs["3. Enter Discord Token & Client Secrets"]
    Inputs --> Build["4. Koyeb Builds Multi-Stage Dockerfile Container"]
