...
import { formatQueue } from '../../lib/music/utils.js';
import { EmbedHandler } from '../../lib/embeds/builder.js';
import type { BotCommand } from '../../handlers/registry.js';

export const queueCommandDef: ApplicationCommand = {
  name: 'queue',
...
export async function handleQueueCommand(
  ...
  const content = formatQueue(player?.queue ?? [], player?.current ?? null);

  return EmbedHandler.for(deps)
    .primary()
    .title('Music Queue', '📜')
    .description(content)
    .footer(`Loop: ${player?.loop ?? 'none'} • Volume: ${player?.volume ?? 100}%`)
    .respond();
}
