...
export const bonkCommandDef: ApplicationCommand = {
  name: 'bonk',
...
export async function handlebonkCommand(
  ...
  const gifUrl = await client.getCategoryGif('bonk');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${actor?.id ?? 'someone'}> bonked <@${targetId}>!` : '*bonked*';
