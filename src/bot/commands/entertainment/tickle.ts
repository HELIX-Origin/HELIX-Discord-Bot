...
export const tickleCommandDef: ApplicationCommand = {
  name: 'tickle',
  description: 'Tickle someone with a GIF',
  options: [
    {
      name: 'user',
...
export async function handletickleCommand(
  ...
  const gifUrl = await client.getCategoryGif('tickle');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${actor?.id ?? 'someone'}> tickled <@${targetId}>!` : '*tickled*';
...
export const tickleCommand: BotCommand = {
  def: tickleCommandDef,
  category: 'entertainment',
  isEnabled: (deps) => Boolean(deps.config.features.gifsEnabled && deps.config.klipyApiKey),
  execute: handletickleCommand,
};
