...
Deploy HELIX Discord Bot to [Render](https://render.com/) on the **100% Free Web Service tier** using the native Node.js 22 runtime and Blueprint configuration (`render.yaml`).
...
- **Runtime**: Native Node.js 22 (`npm ci && npm run build` $\rightarrow$ `npm start`)
...
flowchart TD
    ...
    Inputs --> Build["4. Render Installs & Compiles TypeScript via npm run build"]
