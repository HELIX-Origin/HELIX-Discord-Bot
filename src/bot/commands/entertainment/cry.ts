...
export const cryCommandDef: ApplicationCommand = {
  name: 'cry',
  description: 'Show a crying reaction GIF',
  options: [
    {
      name: 'user',
...
export async function handlecryCommand(
  ...
  const gifUrl = await client.getCategoryGif('cry');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${actor?.id ?? 'someone'}> crying <@${targetId}>!` : '*crying*';
