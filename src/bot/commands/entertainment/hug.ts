...
export const hugCommandDef: ApplicationCommand = {
  name: 'hug',
  description: 'Hug someone with a GIF',
  options: [
    {
      name: 'user',
...
export async function handlehugCommand(
  ...
  const gifUrl = await client.getCategoryGif('hug');
  if (!gifUrl) {
  ...
  const description = targetId ? `<@${actor?.id ?? 'someone'}> hugged <@${targetId}>!` : '*hugged*';
