...
export const resumeCommandDef: ApplicationCommand = {
  name: 'resume',
...
export async function handleResumeCommand(
  ...
  if (!player || !player.current || !player.paused) {
    return EmbedHandler.for(deps).error().title('Cannot Resume').description('Playback is not paused.').respond(true);
  }
  await manager.pause(guildId, false);
  return EmbedHandler.for(deps).primary().title('Resumed', '▶️').description('Playback has been resumed.').respond();
}

export const resumeCommand: BotCommand = {
  def: resumeCommandDef,
  category: 'music',
  isEnabled: (deps) => Boolean(deps.config.features.lavaEnabled),
  execute: handleResumeCommand,
};
